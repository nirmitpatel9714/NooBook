    ...
    if args.iter().any(|a| a == "nbmode") {
        ...
        if hist.commands.is_empty() {
            ...
            for cmd in hist.commands.iter().rev().take(50) {
                println!(
                    "#{} [{}] {}  --  {}",
                    cmd.id, cmd.language, cmd.command, cmd.timestamp
                );
            }
        }
...
fn parse_lang_command(input: &str, config: &config::ConfigMap) -> Option<(String, String)> {
    ...
    if let Some(close) = trimmed.find(')')
        ...
        && close > open + 1
        && config.contains_key(trimmed[..open].trim())
        && !trimmed[open + 1..close].is_empty()
    {
...
async fn run_cli(app: &mut App, startup: &[String]) -> Result<(), Box<dyn Error>> {
    ...
    loop {
        ...
        let prompt = format!(
            "\x1b[32;1m➜ \x1b[36;1m[{}]\x1b[0m \x1b[33m({})\x1b[0m \x1b[35m❯\x1b[0m ",
            dir_name, lang
        );
            ...
            match parts[1] {
                ...
                "history" => {
                    ...
                    if hist.commands.is_empty() {
                        ...
                        for cmd in hist.commands.iter().rev().take(50) {
                            println!(
                                "  \x1b[36m[{}]\x1b[0m {}  --  {}",
                                cmd.language, cmd.command, cmd.timestamp
                            );
                        }
                    }
                    continue;
                ...
                "sessions" => {
                    ...
                    if sessions.is_empty() {
                        ...
                        for s in &sessions {
                            let cell_total: usize =
                                s.workspaces.iter().map(|w| w.cells.len()).sum();
                            println!(
                                "  \x1b[33m{}\x1b[0m  |  {} workspaces, {} cells",
                                s.name,
                                s.workspaces.len(),
                                cell_total
                            );
        ...
        terminal.draw(|f| {
            ...
            // ── workspace tab bar ──
            {
                ...
                for (i, ws) in app.workspaces.iter().enumerate() {
                    ...
                    tab_spans.push(ratatui::text::Span::styled(ws_label.to_string(), style));
                }
                tab_spans.push(ratatui::text::Span::raw("   "));
    ...
    let res: io::Result<()> = loop {
        terminal.draw(|f| {
            ...
            let list_block =
                Block::default()
                    .borders(Borders::ALL)
                    .title(if tab == ManageTab::Sessions {
                        " Saved Sessions "
                    } else {
                        " Command History "
                    });
                        ...
                        for (i, cmd) in hist.commands.iter().rev().skip(start).take(20).enumerate()
                        {
                            ...
                            list_lines.push(ratatui::text::Line::from(
                                ratatui::text::Span::styled(
                                    format!(
                                        "{}{} [{}]  {}",
                                        prefix, cmd.command, cmd.language, cmd.timestamp
                                    ),
                                    style,
                                ),
                            ));
